...
function AuthProvider(props) {
  ...
  const login = async (credentials) => {
    ...
    const token = result_login.data.token;
    localStorage.setItem("token", token);
    const userDataFormUser = jwtDecode(token);

    setState({
      ...state,
      user: userDataFormUser,
    });
  ...
  const register = async (data) => {
    ...
    //  ที่สร้างไว้ด้านบนพร้อมกับ Body ที่กำหนดไว้ในตารางที่ออกแบบไว้
    await axios.post("http://localhost:4000/register/register", data);
    navigate("/login");
  };
